...
open class Iden3IdentityFactory {
     ...
     */
    fun initialize(web3Url: String, storePath: String, checkTicketsPeriod: Long) : Boolean {
     ...
     * @param eventCallback: Callback of events associated to the identity.
     *
     * @return The new Iden3 identity created.
     */
    @Throws(Exception::class)
    fun createIdentity(alias: String, password: String, eventCallback: Iden3EventCallback?) : Iden3Identity? {
        if (isInitialized()) {
            if (alias.isEmpty() || !isDigitsOrLetters(alias) || password.isEmpty()) {
                ...
                try {
                    ...
                    return Iden3Identity(Iden3mobile.newIdentity(
                        ...
                        checkTicketsPeriod,
                        null
                    ) { event -> eventCallback?.onEventReceived(Iden3Event(event)) })
                } catch (e:Exception) {
                    throw e
                }
            }
        } else {
            throw IllegalStateException("Iden3 API is not initialized. Please, call initialize method before doing this call")
        }
    }
     ...
     */
    @Throws(Exception::class)
    fun loadIdentity(alias: String, password: String, eventCallback: Iden3EventCallback?) : Iden3Identity? {
        if (isInitialized()) {
